...
search([[Node,ValSoFar,Path]|T],FinalPath,FinalCost,KB) :- findall([X,NewCost,[X|Path]], (arc(Node,X,Arcost,KB),NewCost is Arcost+ValSoFar), Children),
                                                           add-to-frontier(Children,T,New), 
